...
$sql = "DELETE FROM btc WHERE id = '$id'";
...
if (isset($_POST['approve'])) {
	...
	if ($status == "Completed") {
		...
		$tdate = date("Y-m-d");
		$query = mysqli_query($link, "UPDATE btc SET status = 'Completed' WHERE id = '$id' ");
		if ($query) {
			...
			$msg = "Deposit Approved Succesfully";
			$subject = "Deposit Completed";
	        $body = "<h3>Hello ".$name.",</h3> <p>Your Deposit request of $".$amount." via ".$method." has been completed</p> ";
...
<?php 
    if ($msg != "") {
        ...
        echo pageRedirect("2", "deposit.php");
    }
    if ($err != "") {
